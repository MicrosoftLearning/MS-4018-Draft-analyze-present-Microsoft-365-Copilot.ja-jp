...
1. ブラウザーから「+++https://word.new+++」と入力して新しい Word 文書を開きます。
...
1. 文書参照をプロンプトに追加するために、スラッシュ `/` を手入力してファイル検索ダイアログを開いてから **Promotion Plan for Chai Tea in Latin America.docx** を検索して選択します。 または、**[コンテンツの追加]** を選択してこのファイルをドロップダウン リストから選択します (OneDrive で使用可能になっている場合)。

1. 右方向キーを選択してプロンプトを送信し、応答を **[生成]** します。
...
1. この段落をハイライトし、表示されたフローティング ツール バーの **[Copilot で編集]** を選択します。
...
1. **[Copilot で下書き]** (鉛筆のアイコン) を使用して Copilot に「プロジェクトのマイルストーンとそれぞれの期限のリストを追加してください」と依頼します。 **[Generate] \(生成)** を選択します。

1. **[保持する]** を選択して、プロジェクト計画にセクションを追加します。

1. このリストをハイライトし、表示されたフローティング ツール バーの **[Copilot で編集]** を選択します。

1. **[テーブルとして視覚化]** を選択または入力します。 全体として良い結果に見えますが、タスクがいつ無事に完了するかを示す列があることを確認しましょう。

1. Copilot プロンプト ボックスに次のプロンプトを入力します。

    「タスク完了日」という 3 番目の列を追加してください。」

1. **[保持する]** を選択して、ドキュメントに表を挿入します。

1. 表の形式を確認し、必要な調整を行います。 その後、[プロジェクトのマイルストーンと期限] セクションから箇条書きを削除できます。
...
1. **[ホーム]** タブの **[Copilot]** を選択して [Copilot] ペインを開いてから、次のプロンプトを入力します。

    +++プロジェクト レポート Promotion Plan for Chai Tea in Latin America ドキュメントを要約してください。 上位 3 つのポイントを強調表示してください。+++

1. 生成された要約を確認し、この要約全体を文書に含めたい場合は **[文書に追加]** (出力の下にあるプラス記号) を選択します。 テキストの任意の部分を強調表示し、コピーしてドキュメントに貼り付けることもできます。

> [!IMPORTANT]
> Copilot が生成したテキストは、ドキュメント内のカーソルの位置に挿入されます。 コンテンツの挿入を選択する前に、ドキュメントの末尾に移動していることを確認します。

必要に応じて、手動によるテキストの調整や要約コンテンツの強調表示を行ったり、Copilot を使用して **[自動書き換え]** を行ったりすることができます。 また、プロジェクト レポートを関係者と共有するときに、この要約を基盤として Teams または Outlook メッセージを作成することもできます。
